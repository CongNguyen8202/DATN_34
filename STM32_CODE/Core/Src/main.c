...
uint32_t AD[1];
uint8_t buffer[1]; 
int  mode, trang_thai_thiet_bi, trang_thai_thiet_bi_cuoi_cung, tb1, tb2, tb3, tb4, rx_index;
float nhiet_do, do_am, do_am_dat, cuong_do_sang;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
  if (huart->Instance == USART1) {
    if (buffer[0] == '@') {
      // Nh?n d? li?u t? ESP8266
      HAL_UART_Receive(&huart1, (uint8_t*)&nhiet_do, sizeof(nhiet_do), HAL_MAX_DELAY);
      HAL_UART_Receive(&huart1, (uint8_t*)&do_am, sizeof(do_am), HAL_MAX_DELAY);
      HAL_UART_Receive(&huart1, (uint8_t*)&do_am_dat, sizeof(do_am_dat), HAL_MAX_DELAY);
      HAL_UART_Receive(&huart1, (uint8_t*)&cuong_do_sang, sizeof(cuong_do_sang), HAL_MAX_DELAY);
    }
    // Ti?p t?c nh?n mã b?t tay
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	mode = 0;
	LCD_xy(0, 0);
	LCD_ghi_chuoi("     NHOM 34    ");
	LCD_xy(1, 0);
	LCD_ghi_chuoi("Nguyen Quy Cong ");
	LCD_xy(2, 0);
	LCD_ghi_chuoi("Nguyen Duy Khanh");
	LCD_xy(3, 0);
	LCD_ghi_chuoi("  Cu Van Duong  ");
	HAL_Delay(1000);
  /* USER CODE END 2 */
